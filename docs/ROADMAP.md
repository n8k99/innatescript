...
Innate is a Common Lisp interpreter for a choreographic programming language — markdown files containing Innate expressions that are simultaneously readable documents and executable multi-agent choreographies. The build order is dictated by compile-time dependencies: packages → conditions → AST → tokenizer → parser → environment → resolver protocol → stub resolver → evaluator → REPL. Each phase delivers one coherent, loadable, testable layer.

v1.0 shipped 2026-03-29 with 9 phases, 19 plans, and 97+ passing tests. The interpreter handles the full Innate expression grammar including tokenization, parsing, resolver protocol, stub resolver, two-pass evaluation, commission/fulfillment, and an interactive REPL with file runner. Phases 10-12 below extend the language with choreographic semantics.
...
- [x] **Phase 3: Tokenizer** - Full Innate symbol vocabulary, wikilink disambiguation, prose detection (completed 2026-03-28; choreographic tokens `<-`, `concurrent`, `join`, `until`, `sync`, `at` deferred to Phase 10)
- [x] **Phase 4: Parser** - Recursive descent, prose passthrough, complete expression grammar (completed 2026-03-28; three-bracket limit enforcement deferred to Phase 12)
- [x] **Phase 5: Resolver Protocol and Environment** - defgeneric boundary, evaluation environment struct (completed 2026-03-28 with 6 generics; `deliver-verification` and `schedule-at` deferred to Phase 10)
- [x] **Phase 6: Stub Resolver** - In-memory stub resolver conforming to v1 resolver protocol (completed 2026-03-28; choreographic method specializations deferred to Phase 10)
- [x] **Phase 7: Evaluator Core** - Two-pass hoisting, etypecase dispatch, prose/heading passthrough (completed 2026-03-29; bracket depth enforcement deferred to Phase 12)
- [x] **Phase 8: Commission and Fulfillment Evaluation** - Agent commission, search, emission, wikilink, bundle, fulfillment operator (completed 2026-03-29; `<-` verification operator deferred to Phase 10)
...
- [ ] **Phase 10: Choreographic Lexing and Parsing** - `<-` verification, choreographic tokens, resolver protocol extensions
- [ ] **Phase 11: Choreographic Coordination** - `concurrent`, `join`, `until`, `sync`, `at` evaluation and projection
- [ ] **Phase 12: Choreographic Integration** - Three-bracket enforcement, projection, decree migration, end-to-end choreographic tests
...
**Plans**: 2 plans (completed; originally Phase 9 in v1.0 execution)
...
**Goal**: The five coordination primitives (`concurrent`, `join`, `until`, `sync`, `at`) evaluate correctly, enabling multi-agent choreographies with parallel execution, synchronization, time-bounded obligations, and temporal triggers
**Depends on**: Phase 10
